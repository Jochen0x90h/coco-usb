...
/**
 * Interface for USB device support (device side, not host side)
 *
...
public:
    ...
    /**
     * Wait for a request to the device (e.g. control request for USB)
    ...
    /**
     * Get the current setup packet. Call after waiting for a request (co_await device.untilRequest();)
     */
